...
public class Player extends Entity {
	...
	public Player(int x, int y, int width, int height, BufferedImage sprite) {
		...
		for (int i = 0; i < 4; i++) {
			rightPlayer[i] = Game.spritesheet.getSprite(32 + (i * 16), 0, 16, 16);// calculo realizado para seguir a
																					// sequencia de sprites no paint.net
		}
	...
	public void tick() {
		moved = false;
		if (right && World.isFree((int) (x + speed), this.getY())) {// verifica na hora do personagem mover-se, se há
																	// algum muro ou objeto estatico, se houver, não
																	// passa!
			moved = true;
			dir = right_dir;
			x += speed;
