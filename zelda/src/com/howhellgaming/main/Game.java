...
import com.howhellgaming.com.entities.Enemy;
import com.howhellgaming.com.entities.Entity;
import com.howhellgaming.com.entities.Player;
...
public class Game extends Canvas implements Runnable, KeyListener {
	...
	public static final int WIDTH = 320;// 320final porque é uma constate e não sofre alteração
	public static final int HEIGTH = 180;// 180
	...
	public Game() {
		rand = new Random();
		addKeyListener(this);// this porque o Listener está nesta classe, se fosse outra classe teria que
	...
	public synchronized void stop() {// certifica que quando ocorrer algum erro no jogo, todas as threads sejam
										// finalizadas!
		isRunning = false;
		try {
			thread.join();
	...
	public void render() {// Método dos graficos
		...
		// Graphics2D g2 = (Graphics2D) g;//isso transforma a variável g em Graphics
		// 2D, o que permite tecnicas mais avançadas tipo animação e etc...
		world.render(g);
		for (int i = 0; i < entities.size(); i++) {
			Entity e = entities.get(i);
			e.render(g);
	...
	public void run() {// looping para o jogo estar em constante running.
		long lasTime = System.nanoTime();// pega o tempo atual do nosso computador em nano segundos, isso da mais
											// precisao nos fps de maneira profissional
		...
		double delta = 0;
		int frames = 0;
		double timer = System.currentTimeMillis();// retorna o tempo do computador, usamos isso para contagem do fps,
													// mas eh opcional.
		requestFocus();// Metodo que foca a tela automaticamente ao abrir o jogo, sem precisar clicar
						// na janela para tal.
		while (isRunning) {
